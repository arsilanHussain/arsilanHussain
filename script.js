...
// Set Leaflet's default marker icon paths to use the `libs` directory
...
function populateCurrencyDropdown() {
    $.ajax({
        url: 'get_all_currencies.php',
        type: 'GET',
        success: function(response) {
            if (response && response.success && response.currencies) {
                const currencyDropdown = document.getElementById('targetCurrency');
                currencyDropdown.innerHTML = '<option value="" selected disabled>Select Currency...</option>'; 
                ...
                console.error('Failed to load currencies:', response.message || 'Unexpected response structure');
            }
        },
        error: function(xhr, status, error) {
...
document.getElementById('convertCurrency').addEventListener('click', function() {
    ...
    if (!targetCurrency) {
        document.getElementById('currencyInfo').innerHTML = '<p>Please select a currency to convert to.</p>';
        return;
    }
